...
| id | triagem | nota |
|---|---|---|
| D2-001 | **agir agora** | destrutiva: DROP do wrapper + `__nucleo` · **0270 APLICADA** em 10/09 (destrutiva, TTY do Yan) · `DROP` do wrapper e do núcleo (`0270:93-94`): `public.get_fluxo_caixa_kpis_diario()` e `…__nucleo()`. O bloco-guarda da própria migration aborta a transação se qualquer um dos 13 alvos sobreviver ao `DROP` — proteção contra assinatura errada, que é **no-op silencioso**. |
| D2-002 | **agir agora** | destrutiva: DROP das 4 `__nucleo`; reconfirmar no ato que o wrapper não faz `EXECUTE format` · **0270 APLICADA** em 10/09 (destrutiva, TTY do Yan) · `DROP` das 4 `__nucleo` (`0270:114-117`): `get_gerencial_lancamentos__nucleo(integer)`, `…_planilha__nucleo()`, `…_projecao_diaria__nucleo(integer)`, `…_saldos__nucleo()`. Reconfirmado no ato que nenhum wrapper as chama por `EXECUTE format`. **Uma assinatura estava errada no rascunho** (`projecao_diaria__nucleo(integer, integer)` contra o real `(integer)`) — `DROP … IF EXISTS` com tipo errado não erra, apenas não faz nada; pego antes de aplicar e é a razão do bloco-guarda. |
| D2-003 | **agir agora** | destrutiva · **0270 APLICADA** em 10/09 (destrutiva, TTY do Yan) · `DROP` de `app.current_user_setor_id()` e `public.get_my_profile()` (`0270:151-152`). ⚠️ O `get_my_profile` estava **reservado** pelo ADR-0107 para "decisão futura do usuário" — a remoção é o exercício dessa reserva, pedida nominalmente pelo Yan, não descuido. O `revisor-db` levantou como ALTO e com razão; registrado na Decisão 3 do ADR-0173. |
| D2-004 | **agir agora** | destrutiva · **0270 APLICADA** em 10/09 (destrutiva, TTY do Yan) · `DROP` de `app.is_financeiro()` (`0270:159`) — predicado coberto pelo RBAC dinâmico via `app.exigir_acesso`. |
| D2-005 | **agir agora** | destrutiva, **com** confirmação no ato de que nada lê `app.config` direto · **0270 APLICADA** em 10/09 (destrutiva, TTY do Yan) · `DROP` de `app.get_config_numeric(text)` (`0270:172`), **depois** de confirmar no ato que nada lê `app.config` direto — era a condição que a própria triagem impôs. |
| D2-006 | **agir agora** | **manter** a função; `COMMENT ON FUNCTION` declarando "kill switch dormente — runbook v4-13" na aditiva · **0269 APLICADA** em 10/09 (aditiva, sob backup-gate) · a função **FICA**, como a triagem decidiu. Ganhou `COMMENT ON FUNCTION` (`0269:103`) declarando-a kill switch dormente, sem chamador **por desenho**, operada pelo runbook `v4-13`, com a frase "NÃO REMOVER por análise estática de código morto" — o comentário existe para que a próxima varredura de código morto não proponha o `DROP` de novo. |
| D2-007 | **agir agora** | decisão do Yan: **dropar**. Destrutiva leva as 3 RPCs **+ `app.meta_subsetor` e `app.meta_subsetor_historico`** (provar vazias no ato); depois **fechar o PR #213**. `get_sumario_subsetor` e a chave `produtos_nao_classificados` FICAM (vivas na Performance) · **0270 APLICADA** em 10/09 (destrutiva, TTY do Yan) · `DROP` das 3 RPCs (`0270:190-192`) **+** `app.meta_subsetor_historico` e `app.meta_subsetor` (`0270:215-216`), as duas provadas **vazias no ato** (0 linhas, 0 FKs, 0 policies, 0 triggers) com guard que abortaria se tivessem ganhado linha entre a prova e a aplicação. **PR #213 fechado** com comentário apontando este item; o corpo das 3 RPCs fica preservado em `_insumos/catalogo-funcoes-def.txt` e o histórico da branch, no repositório. `public.get_sumario_subsetor` — nome parecido, função diferente — **sobreviveu**, e o guard a checa explicitamente. |
...
| D2-010 | **agir agora** | aditiva: REVOKE/GRANT explícito nas 21 · **0269 APLICADA** em 10/09 (aditiva, sob backup-gate) · **o achado errava o número e o alvo: são 8, não 21.** A consulta da Fase 1 não olhava o grant DEFAULT para `PUBLIC`, então reportava "sem grant" justamente onde o grant era o **pior** (PUBLIC inclui `anon`). Das 21, 12 já estavam corretas e dar-lhes o `GRANT` que o item pedia teria **alargado** acesso; e `app.norm_nome`, a mais chamada das 8, não estava na lista. `REVOKE` aplicado nas 8 de ACL default (`0269:71-78`); a 9ª linha de REVOKE (`0269:239`) é o re-REVOKE de `app.exigir_acesso` depois do `CREATE OR REPLACE`, que reseta grants. Verificado via REST/service_role, com caso de contrato. |
| D2-011 | descartar | check limpo |
| D2-012 | descartar | check limpo |
| D2-013 | **agir agora** | destrutiva: DROP de `dim_taxa_cdi_taxa_plausivel` (±100%), mantendo a de ±5% · **0270 APLICADA** em 10/09 (destrutiva, TTY do Yan) · `ALTER TABLE analytics.dim_taxa_cdi DROP CONSTRAINT IF EXISTS dim_taxa_cdi_taxa_plausivel` (`0270:228`) — sai a de ±100%, fica a de ±5%, que é a restritiva. O guard da migration confere que a sobrevivente continua lá (contrapeso: o DROP certo não pode levar a errada junto). |
...
| D8-005 | **agir agora** | **resolvido**: critério **"medição fica, opinião sai"** — ver bloco abaixo  · **feito em a152d1c: 12 apagados, não 13.** A condição da própria triagem ("só depois de E1–E8 estarem no `backlog-v6.md`") foi cumprida antes, em d160aa5. Saíram `docs/audits/` (9) e `docs/superpowers/` (3) inteiras, com grep de cada basename = **0 citações**. **`docs/harness/inventario-claude-md.md` FICOU contra a lista**: o grep achou citação viva em `docs/adr/0157:18` e `:74` ("Migração provada por inventário sem órfãos") e em `docs/WORKING-CONTEXT.md:979` — terceira vez neste bloco que a lição do `getPool` pega um alvo. `docs/harness/sonda-disparo.md` ficou pela outra condição: **E5 segue em aberto**. |
...
| D8-008 a D8-011 | **agir agora** | reescrita do `README.md` (entrega da Fase 2), lido inteiro contra o repo limpo  · **feito em 990555f.** O arquivo estava parado em julho/2026 e mentia em quase todo número — título "WT Finance", versão 5.0.0, 165 migrations, 126 ADRs, 29 arquivos de teste, Next 16.2.9, "4 camadas" de enforcement, e uma "limitação conhecida" afirmando que a migration **0176 estava pendente de aplicação** (aplicada há 94 migrations). Reescrito, com cada número conferido no ato. Estrutura nova: o README responde "o que é e como rodo" e manda o leitor novo para o `estado-do-projeto.md`; a tabela "onde está cada coisa" é indexada pela **pergunta**, não pela pasta. |
| D8-012 | **agir agora** | split: `estado-do-projeto.md` (novo) + `WORKING-CONTEXT.md` enxuto ao estado  · **feito.** `docs/estado-do-projeto.md` criado em becb0ff (11 seções: arquitetura em uma tela, módulos × fonte de verdade, os dois regimes e por que a ponte fecha por construção, integrações, 23 ADRs vigentes conferidos um a um, canon × método, as 14 skills, utilitários fora do grafo, rotina de dependências, estado atual). `docs/WORKING-CONTEXT.md` enxugado em 106c91f de **1.252 para ~190 linhas** — saíram as ~880 de histórico por versão (que é git e out-briefing) e **duas afirmações mortas**: citava um `supabase/patches/PENDENTE-*.sql` inexistente e marcava o re-upload da v5.5.2 como pendente, feito em 19/08. |
...
| D8-016 | **agir agora** | manter `v3-6-apendice.md` e **documentar a exceção** de nomenclatura no ADR de fechamento (regra 4: ADR não sai)  · **feito em b6144d6**: a exceção está na **Decisão 5 do ADR-0173**. `v3-6-apendice.md` fica sem prefixo numérico porque **não é um ADR** — é o apêndice de um conjunto de decisões da v3.6; numerá-lo inventaria um ADR que nunca existiu e deslocaria quem procura pela sequência. Nomeado no ADR para não ser "corrigido" por varredura futura. |
| D8-017 | descartar | regra 4 cumprida |
| D8-018 | **agir agora** | varredura completa dos 42 hits; marcar os que faltarem — **feito.** Primeiro a contagem: eram **108 hits em 55 arquivos**, não 42 (o achado subcontou). Lidos um a um, no entorno: **10 relações reais** (o texto aponta outro ADR por número) e **93 de ruído** — "substitui" de dado, componente, cor, lint, fluxo, ou `revoga` de GRANT; e as "Emenda 1/2" que um ADR faz a si mesmo, que não são relação entre ADRs. Dos alvos reais, **5 já estavam marcados** (0031←0052; 0158/0159/0160/0161←0172) e **7 cabeçalhos foram corrigidos**: **0109**←0153, **0137**←0142, **0142**←0143, **0148**←0152, **0149**←0164 *e* 0165, **0151**←0164, **0164**←0165. Marcador usado: **`Emendado por:`, não `Supersedido por:`** — esses sete continuam vigentes e só tiveram uma parte revista; escrever "supersedido" seria falso e mandaria o leitor descartar decisão viva. O status `aceito` fica. **Observação de convenção** (para o ADR de fechamento): o **ADR-0055** instituiu a marcação **só no lado superado** — por isso 0051/0042/0048 não declaram "eu supersedo X", e isso é a convenção funcionando, não uma falha. O 0172 faz os dois lados. **Caso sem ação:** `0053:6` tem um `Supersede:` formal cujo alvo é um componente de UI (o filtro global do canto superior direito), não um ADR — não há arquivo para corrigir. · prova: `grep -rl "Emendado por\|Supersedido por" docs/adr/` passa de 7 para 14 arquivos |
| D8-019 | descartar | numeração histórica |
| D8-020 | descartar | nenhuma lição falsa na amostra |
| D8-021 | **agir agora** | verificar a skill `ingestao-planilhas` (fecha o invariante 4) — **feito: skill VERIFICADA, zero lição falsa.** As 7 seções conferidas contra o repo, uma afirmação por vez: §1 parse de arquivo no servidor via API Route (`src/app/api/gerencial/import/route.ts`, `src/app/api/admin/upload-lancamentos/route.ts`); §2 Web Worker (`src/lib/carga/parse-em-worker.ts`); §4 módulo único de coerção com os 4 exports que a skill cita — `toNum`, `toIsoDate`, `toCentavos`, `toStr` (`src/lib/carga/coercao.ts:21,56,100,122`) e o lint `eslint-rules/no-coercao-reimpl.mjs`; §5 `vendas-parser.ts` é mesmo isomórfico (cabeçalho declara "núcleo ISOMÓRFICO", sem diretiva de cliente) e as 4 RPCs do pipeline seguem vivas, **e a ressalva sobre o caminho antigo continua verdadeira** — `truncate_dynamic_tables` e `inserir_lote_raw` seguem usadas por `supabase/seed/seed.ts:50,82` (os 6 scripts removidos no Bloco 1 eram outros; o `seed.ts` ficou), e a 0270 não as dropou; §7 a receita do CSV bate linha a linha com `src/lib/patrimonio/csv.ts` (BOM, `EOL='\r\n'` :18, escape de `[";\r\n]` :27, decimal com vírgula `toFixed(2).replace('.', ',')` :45, junção por `;` :71). **Nada apagado — não havia lição falsa a apagar.** A skill ainda GANHOU conteúdo neste bloco (§5, operação da carga, migrada do runbook v4-15 em 065c971). **Fica em aberto:** a verificação de `react-padroes`, que o D8-021 também mencionava e que não estava no pedido do Bloco 6 — vai para o backlog v6. |
| D8-022 | **agir agora (ato humano)** | diff do hook PreToolUse para `git add -A`/`-a` pronto no PR; aplicação é do Yan (`.claude/hooks/` protegido) · diff do hook `PreToolUse` para `git add -A`/`-a` pronto no PR — **APLICADO pelo Yan em 13/09** e versionado em `0819c91`. `.claude/hooks/protecao-git-add.mjs` instalado (idêntico byte a byte ao artefato) e registrado num matcher `Bash` novo, sem tocar o do `protecao-config`. A regra deixa de ser prosa do `CLAUDE.md` e vira destino 1 da régua de 5. ⚠️ **O `revisor` do fechamento achou 3 FALSOS NEGATIVOS na versão instalada**, os três confirmados ao vivo: `bash -c 'git add -A'` (a remoção de todo texto entre aspas apagava o próprio comando — vale para `sh -c`, `eval`, `ssh host '…'`), `git add -- .` (o `--` idiomático escapava do padrão) e `git add -vA` (o cluster de flags exigia `a` **minúsculo**, sem flag `i` — e a documentação AFIRMAVA cobrir `-vA`). Corrigido: a neutralização passa a valer só para o argumento de `-m`/`--message`, e as aspas restantes viram espaço, de modo que o conteúdo do interpretador fica visível ao scanner. **Bateria de 21 → 31 casos**, cobrindo os três. A lição vale mais que o defeito: *caso de teste que nasce depois da regex só confirma a regex* — a bateria existia para pegar isso e não pegou, porque testava o que o autor já sabia que funcionava. |
...
| D9-015 | **agir agora** | aditiva: `CREATE OR REPLACE` do **catálogo vivo**, trocando só o texto do `RAISE` · **0269 APLICADA** em 10/09 (aditiva, sob backup-gate) · `CREATE OR REPLACE FUNCTION app.exigir_acesso` (`0269:175`) extraído do **catálogo vivo** via `pg_get_functiondef`, trocando **só** o texto do `RAISE` para `'USUARIO_INATIVO: sem cadastro ativo no Janus'` (`0269:215`), com diff linha a linha no header. **O achado dizia "as duas funções"; é UMA** — as migrations 0119 e 0133 contêm o texto, mas a 0133 é reaplicação: ler migration em vez do catálogo conta duas vezes o mesmo objeto. Forçado o caminho `USUARIO_INATIVO` com uid inexistente para ver o texto novo; caso de contrato em `rpc-contrato.test.ts`. |
...
| id | triagem | nota |
|---|---|---|
| D10-001 | **agir agora (ato humano)** | **criar** o `deny` de `npx supabase db push` cru + os `allow` estreitos dos gates. O `db-gate` vive no wrapper `npm run db:migrate`; `db push` direto passa **por fora** de classificação, backup-gate e restore-test, e aplica todo o pending — inclusive destrutiva estacionada. Hoje a única proteção é disciplina. Diff pronto no PR; aplicação é do Yan · criar o `deny` do `db push` cru + os `allow` estreitos — **APLICADO pelo Yan em 13/09**, versionado em `0819c91`. **9 `deny` no `~/.claude/settings.json` global** (`supabase db push` cru nas duas formas, `db:migrate -- --destrutiva` com `:*` e exata, `git push --force`/`-f`/`--force-with-lease`/`origin main`/`origin HEAD:main`) e **23 `allow` no `.claude/settings.json` do projeto**, que é **versionado** — de propósito, porque `settings.local.json` é git-ignored **e por diretório**, então worktree não o herda (foi essa diferença que negou dois comandos legítimos em 10/09). **Verificado ao vivo:** `npx supabase db push --dry-run` volta negado. **Achado extra no caminho:** o `.claude/settings.json` do checkout raiz estava com **JSON inválido desde 28/07** — tentativa antiga de aplicar isto à mão que falhou em silêncio e provavelmente derrubou os hooks de lá por seis semanas; revertido para a versão commitada. |
| D10-002 | **agir agora** | nota no `estado-do-projeto.md`: 14 pastas (9 domínio + 3 rituais + 2 externas) · nota no `estado-do-projeto.md`: 14 pastas (9 domínio + 3 rituais + 2 externas) — **feito em becb0ff**, §8 do arquivo, com as 9 de domínio e os 3 rituais nomeados um a um e as 2 externas (`web-artifacts-builder`, `web-design-guidelines`) marcadas como não sendo do projeto e não seguindo suas convenções. |
