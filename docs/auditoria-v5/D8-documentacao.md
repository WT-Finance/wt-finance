...
| D8-006 | `docs/faturamento-legado/` (6 scripts py/R do processo manual pré-Asaas) — sem citação; suplantado pelas Fases 1–4b de Faturamento (v4.30–4.37) | grep `faturamento-legado` em CLAUDE.md/docs/**: 0 hits fora da pasta — **ESTA EVIDÊNCIA É FALSA** (ver D8-006 no relatório triado: 5 citações vivas em ADR-0135/0136/0140 e em `src/lib/asaas/*`, `src/lib/email/template.ts`). A varredura do explorador não alcançou `docs/adr/` nem `src/` | baixo | S | apagar (ou mover para arquivo pessoal do Yan) | apagar | | |
