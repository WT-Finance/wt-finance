# Briefing v5.10.3 — Role `verificador`: a credencial que verifica não escreve

**Tipo:** PATCH (renumerada de v5.11.0 para v5.10.3 por decisão do Yan em 14/09) · **Migration:** **1 aditiva** (role + grants; numerar na hora — próxima livre `0271`) · **ADR:** **1 novo** (próximo livre `0174`, conferir no remoto) — "Separação entre credencial de verificação e credencial de aplicação" · **Base:** `main` (v5.10.2) · **Branch:** `feat/v5-10-3-role-verificador` · **Rota A**
...
1. `docs(v5-10-3): briefing da versao`
