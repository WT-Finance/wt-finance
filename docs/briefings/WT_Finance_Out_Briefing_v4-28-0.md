...
- `src/components/financeiro/calculadora-rateio.tsx` — UI: upload → cruzamento → resumo por setor (cor `SETOR_COLORS`, rótulo **Trips←Lazer só aqui**, valor com sinal + %, barra proporcional) + **Total e fechamento de conta evidentes** + aviso de `ignoradas` + lista linha-a-linha (toggle). **Refino pré-merge (coerência com a Atualização de Dados / `admin/uploads`):** dropzone com **arrastar-e-soltar** + clicar (`onDragOver/Leave/Drop` + estado `isDragging`); **spinner (`Loader2`) + barra de progresso** durante o processamento (2 fases: "Lendo a fatura…" → "Cruzando com a base…", tokens `bg-action-soft`/`bg-action-primary`); **cabeçalho sem ícone** (padrão das demais páginas, `text-xl font-semibold` + subtítulo); erro **inline no dropzone** (não mais faixa separada); container `max-w-2xl` como o upload.
