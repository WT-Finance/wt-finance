...
**Tema:** Refinos de UI de tabelas (transversal) + padronização do cabeçalho no Design System + rótulos de permissão. **1 migration destrutiva (`0167`) — aplicação HUMANA.** **Merge e deploy ficam com o usuário.**
...
2. **Migration só de rótulo** ✅ — `0167` só faz `UPDATE ... rotulo`; não altera chave/FK/permissões; reversível; **não aplicada pelo agente**.
...
## Gate de fechamento
`npx tsc --noEmit` → 0 · `npm run lint` → limpo · `npm test` → verde · `npm run build` → exit 0. **A migration 0167 NÃO foi aplicada** (destrutiva → aplicação humana).
...
4. **Permissões:** aplicar a migration `0167` (`npm run db:migrate -- --destrutiva`) e conferir os novos rótulos no modal de Usuários e Acessos.
...
- **RBAC:** `src/lib/auth/areas.ts` + `supabase/migrations/0167_rotulo_areas.sql` (destrutiva, aplicação humana).
