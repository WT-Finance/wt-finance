...
| ~~B-02~~ | ~~`nodemailer` 9 → 10 (4 CVEs moderate/high: bypass de allow-list de domínio, DoS)~~ — **RESOLVIDO na v5.10.2.** O risco previsto ("major com breaking; ação externa irreversível") **não se materializou no código**: os breaking changes da major são Node >= 20 e build dual ESM/CJS, e nenhum arquivo de `src/` precisou mudar. A parte "ação externa irreversível" foi levada a sério de outro jeito — prova de envio REAL dos 3 templates em modo teste fail-closed, com anexos inline conferidos byte a byte contra um e-mail de produção do nodemailer 9 (a suíte não serve de prova: ela mocka o nodemailer). **`npm audit` foi a zero** | D6-004 | — | — | fechado por execução |
| ~~B-03~~ | ~~`vitest` 3 → 5 (CVE moderate em `@vitest/mocker`)~~ — **RESOLVIDO na v5.10.1.** Dois majors de uma vez, dev-only. O risco previsto ("config e reporters mudam") **não se materializou**: cruzados os guias oficiais 3→4 e 4→5 contra o uso real, nenhum breaking change tocava este repo e `vitest.config.ts`/`vitest.setup.ts` ficaram intocados. As duas sondas (`skipIf` silencioso e teste-que-escreve-no-banco) fazem análise ESTÁTICA do código-fonte, não leem reporter — foram provadas por mutação sob o vitest 5. 1.220 testes idênticos, zero skip. Levou junto a vuln do `esbuild`, como a v5.9.7 previu | D6-005 | — | — | fechado por execução |
| B-04 | Majors pendentes: `typescript` 5.9 → 7, `eslint` 9 → 10, ~~`@types/node` 20 → 22~~ (**feito na v5.10.1**: foi a `^24`, não `^22` — requisito de *peer* do `vitest@5`; `tsc` limpo), `@supabase/ssr` 0.10 → 0.12 | D6-007/008 | alto | L | quebram regras/config; fora do escopo por briefing |
...
| ~~B-22~~ | ~~Baseline de schema + checagem de drift~~ — **FEITO na v6.0.0/M8 (ADR-0178):** `supabase/baseline/schema-v6.json` (79 tabelas, 19 views, 330 funções por assinatura com hash do corpo, as roles de plataforma e as duas roles de máquina com allowlist efetiva, `cron.job`, `pg_default_acl`, extensões) + teste de drift `src/lib/schema-baseline.test.ts`, que reprova qualquer diferença nomeando-a; regenera a cada migration aplicada — ou cron ligado/desligado — no MESMO commit (`npm run db:baseline`) | briefing, invariante 6 | — | — | fechado por execução |
...
## Achados da v6.0.0 que a fundação da ingestão deixou (não couberam nesta versão)

Rastro completo: `docs/briefings/WT_Finance_Out_Briefing_v6-0-0_Fundacao_Ingestao.md` §12 (Pendências
do Yan) e os anexos `anexo-v6-0-0-m{5,7,9}-*.md`.

| # | item | origem | risco | esforço | por que ficou |
|---|---|---|---|---|---|
| B-31 | **GATE 3 — destrutiva** (`DROP` de `truncar_*`/`inserir_lote_*` migrados para as 5 bases + a rota morta `api/admin/upload-lancamentos`) **adiada para a v6.0.1** (decisão do Yan, 25/09). O motivo é a própria pré-condição do GATE 3: "escrita depois que o código que referenciava cada objeto saiu e foi **deployado**" — a v6.0.0 ainda não tinha ido a produção quando a M10 chegou, e uma destrutiva agora apagaria função que a produção viva ainda usa | briefing M10, out-briefing §3 | baixo | S | pré-condição do próprio GATE 3 não estava satisfeita; TTY do Yan, depois do deploy |
| B-32 | A guarda de DATA de `validar_carga_staging`/`promover_carga_vendas` ainda cobre a staging inteira (inclusive as linhas Welcome, que a 0283/M7 já excluiu do `fato_venda`) — uma linha Welcome datada fora de `dim_data` reprovaria a carga toda por um dado que nunca vira fato. Não dispara hoje (`fora_do_range: 0`) | achado MÉDIO do `revisor-db`, M9 | baixo | S | escopar a guarda ao predicado Welcome é migration aditiva pequena, mas não bloqueava a M9 |
| B-33 | Alinhar os dois avisos "1×86" do cruzamento de Operação: o aviso do banco conta LINHAS sem liquidação e sem vencimento (86, inclui as sem número) e o do servidor conta NÚMEROS distintos (1) — duas respostas para a mesma pergunta na mesma tela | anexo M9 §4, out-briefing §12.6 | baixo | S | é rótulo/grandeza, não bug de dado; exige decidir qual das duas contagens a tela deve mostrar |
| B-34 | Pipeline de Weddings órfão (`/api/dashboard/weddings/pipeline`, sem tela) — com `venda_n` agora preenchido pela carga real de Operação, mostraria R$ 49,1 Mi contra R$ 48,4 Mi das outras telas de Weddings | anexo M9 §4, out-briefing §12.5 | médio | S/M | decisão de produto: reativar (e antes rever a regra que diverge) ou aposentar a rota |
| B-35 | Cartas de crédito com vencimento **2049** na base Aberto (5 títulos, −16.762,62 a pagar / +15.861,04 a receber) ficam sem vencimento e saem do `fato_fluxo` pela regra da faixa de data — parecem convenção de "sem prazo" do financeiro, não sujeira do Monde | anexo M9 §2/§4, out-briefing §12.4 | baixo | — | decisão de produto (conversar com a gerente); não é bug de parser |
| B-36 | Aposentar ou portar o card de Pessoas para o caminho novo de ingestão — fecha de vez o **invariante 3** do briefing v6.0.0 ("um parser por base, no servidor; grep de `parseArquivoEmWorker` vazio"), hoje **não cumprido por decisão registrada**: Pessoas (decisão 11, "fica fora — parada, viva") é a única base que ainda parseia no cliente | briefing invariante 3, out-briefing §7 | médio | M | Pessoas está fora do escopo da fundação por decisão explícita do Yan; portá-la é a mesma forma (rota + parser + oráculo) das outras cinco |
| ~~B-37~~ | ~~Ensaio da M5 (checksum falso em transação revertida) como teste permanente~~ — **RESOLVIDO na v6.0.0**: virou `src/lib/ingestao/promover-carga-checksum.test.ts` (0278), quinto caso de `ESCREVEM_E_REVERTEM_HOJE` na sonda `sonda-teste-escreve-banco.test.ts` (skill `banco-e-rpc` §6). A decisão sobre **ambiente de teste próprio** (o gatilho que este caso reforça) segue **aberta à parte** — já registrada em `docs/WORKING-CONTEXT.md`, não duplicada aqui | anexo M5, out-briefing §12.7, skill `banco-e-rpc` §6 | — | — | fechado por decisão do Yan (commitar a prova) |
