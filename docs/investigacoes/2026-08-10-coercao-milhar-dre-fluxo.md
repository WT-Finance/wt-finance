...
Aplicar em `parse-lancamentos-movimentacao.ts` e `parse-titulos-em-aberto.ts`.
...
## 9. ERRATA e desfecho — o que a implementação (v5.5.2) corrigiu deste relatório

**Errata de escopo.** A §8 dizia que `raw: false` estava "nos 9 parsers" e que valia corrigir
todos. **Está errado, e superestimava o alcance.** Aquela contagem era do `raw: false` passado ao
`XLSX.read`, que **não** é o que decide a saída. Quem decide é o `sheet_to_json`, e ali **só os
dois parsers do Fluxo/DRE** passavam `raw: false` — os demais (`parse-pessoas`,
`parse-vendas-produto`, `parse-contas-pagar-receber`, `parse-lancamentos`, `lancamentos`,
`rateio/parse-fatura`) **omitem** a opção, e o default do SheetJS já é `raw: true`. Nenhum outro
parser estava afetado.

**Achado NOVO da implementação, que o relatório não previa: o ramo CSV falhava no sentido
inverso.** Com `XLSX.read(csv, { raw: false })` o SheetJS interpreta o texto com convenção
**americana** antes de qualquer coerção nossa, e `"-1.234,56"` chegava como **−1,23456** (÷1000).
Corrigido junto, com `raw: true` no ramo CSV, devolvendo a string para a regra BR do `toNum`.
Em CSV a ambiguidade de `"-40.933"` é irredutível (texto não tem tipo) e ficou **fixada em teste**
como limitação conhecida — os exports do Monde são `.xlsx`, e esse caminho agora é exato.

**Prova de não-regressão:** 59.139 linhas reais reparseadas campo a campo, antes × depois —
**exatamente 1 campo mudou**, o `valor` da linha corrompida. A soma da base de movimentação foi de
394.492,15 para 435.384,22 (**+40.892,07**, ao centavo o delta que a v5.3.0 atribuiu a
"re-lançamento no Monde").

**Guards entregues** (em `src/lib/carga/parse-fluxo-caixa-valor-nativo.test.ts`, 7 provas, vistas
reprovando o defeito): cobertura pelo caminho do **arquivo** (`parseXxxFile`, não `parseXxxRows` —
