...
| 25–29 | "Manter denso, não só crescer" (podar; PR revisa) | core §8 | — |
| 31–36 | Stack | core §1 | — |
| 38–52 | Comandos essenciais (+ "não inventar `npm run typecheck`") | core §2 | — |
...
| 98–109 | Confirmação no WRAPPER, EOF aborta (ADR-0131) + tokenizer `classificar.mjs` | skill:banco-e-rpc | core §7 mantém 1 linha ("agente não aplica destrutiva por construção") |
...
| 187 | Magic link em 2 passos (nunca consumir token em GET); SMTP próprio | skill:contrato-rpc-front | Precedente v4.13.1 preservado; e-mail de convite → skill:email cross-link |
...
| 291–295 | Workflow §7 PR; NUNCA merge/deploy | core §7 (barreira) + ritual:fechamento-versao | — |
...
| 352–366 | Regras de paralelização (editores puros; orquestrador serializa git/build/banco) | skill:orquestracao | Regra dura repetida no core §7 (barreira: subagentes não rodam git/build/banco/servidor) |
...
| 393–402 | Worktrees: limpar após merge | ritual:pos-merge | + `git pull --ff-only` na raiz (feedback registrado) |
...
| 418–425 | Hook `contexto-sessao` + escape geral | core §7 (M3) | Nova subseção "terceira camada" (classificador + allow) e protocolo D5 entram junto |
...
| 472–478 | Responsividade (cards h-full, tabelas estreitas, eixo Y, sticky no ListDrawer) | skill:ui-design-system | Tabela estreita → tabela-densa; eixo → graficos (cross-links) |
| 480–500 | Definition of Done (checklist) | ritual:fechamento-versao | core §4 aponta o ritual; itens continuam TODOS vivos no ritual |
...
- Faixas contíguas de 1 a 518, sem lacuna (linhas em branco/`---` pertencem ao bloco anterior).
...
| skill:banco-e-rpc | 16 |
| skill:contrato-rpc-front | 8 |
| skill:ui-design-system | 12 |
| skill:tabela-densa | 2 |
| skill:graficos | 2 |
| skill:react-padroes | 2 |
| skill:email | 2 |
| skill:ingestao-planilhas | 6 |
| skill:orquestracao | 6 |
| ritual:nova-versao / fechamento-versao / pos-merge | 7 |
| hook-permissão (enforcement citado como cobertura) | 3 (445–446, 458, 413–417 — mantidos documentados) |
| deletar (substituído por decisão) | 1 (259–261 → D-05) |
