...
### 7. E-mail do tomador exigido pela NFS-e (verificado no sandbox) — tratamento PERMISSIVO
O 1º teste no sandbox retornou *"E-mail do cliente incompleto"* — mensagem da **API do Asaas** (não do nosso código; surge via `errors[0].description`). O e-mail do tomador é, portanto, **exigido** para autorizar a NFS-e (o comentário do script legado "não é obrigatório na NFS-e" era otimista). Decisão do Yan = **permissivo** (o Asaas pode já ter o e-mail no cadastro do cliente mesmo sem ele na base do Monde — não faz sentido bloquear na tela por um dado que a API talvez já tenha): (a) `ensureCustomer` (sob a flag NF) completa o **e-mail** de um cadastro existente a partir da base quando o Asaas não tem (antes só completava o endereço no cadastro achado por CPF/CNPJ — gap); (b) a **prontidão-NF NÃO** exige e-mail (`CPF/CNPJ + endereço + CEP`) — a emissão tenta e o **Asaas valida** o e-mail; sem e-mail em lugar nenhum, a NF falha na emissão com a mensagem clara do Asaas (falha parcial). Evita o falso negativo de bloquear na tela um cliente cujo e-mail só existe no Asaas. **effectiveDate = sempre hoje** (não a coluna Emissão — o Asaas recusa data anterior à atual).
