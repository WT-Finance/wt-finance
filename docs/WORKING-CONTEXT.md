...
- **Migrations 0199–0203 (v5.2.1) APLICADAS em produção** (23–24/07, pelo Yan; `--aditiva --fora-de-ordem`, gate verde). Objetos verificados por introspecção (tabela do diário, triggers diário+broadcast, policy `realtime.messages`, 8 funções, overloads da trava, token no read). **0203** = fix pós-aplicação (`max(uuid)` no histórico → `max(usuario_id::text)::uuid`; Postgres não agrega uuid). **Realtime confirmado FUNCIONANDO** (teste `realtime.send` → aviso ao vivo no app). As 0185–0198 (v5.2.0) idem.
...
- **Ações do Yan p/ fechar a v5.2.1:** migrations aplicadas + Histórico e Realtime confirmados funcionando. Falta: (1) **remover as cópias untracked 0950–0954** de `supabase/migrations/` ANTES do merge (ficam durante o checkpoint pois todo `db:migrate` exige que o remoto exista local; sair só na hora do merge); (2) opcional — teste com 2 usuários reais via `npm run dev` (o mecanismo já foi confirmado ponta-a-ponta); (3) marcar PR #192 pronto + mergear (back-end já no ar via migrations; deploy do front é no merge).
- ⚠️ **`realtime.messages` tem policy nova (`gerencial_broadcast_leitura`)** — não é migration versionada só do app; é objeto do schema `realtime`. Aplicada por 0201; se um reset/rebuild do projeto não a recriar, reaplicar a 0201.
