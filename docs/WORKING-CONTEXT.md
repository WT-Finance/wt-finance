...
- **v5.2.1 FECHADA do lado do Code — resta só o MERGE do Yan (PR #192).** Migrations 0199–0203 em produção + Histórico e Realtime confirmados; todos os ajustes de UI do checkpoint aplicados (refresh não-destrutivo, portal do drawer, máscara/sem-setas na coluna Valor, respiro das rolagens, ícones). Cópias untracked 0950–0954 **já removidas**. Gates: tsc 0 / lint 0 / 469 testes / build OK. Back-end já no ar via migrations; o deploy do front sai no merge. **O Code não mergeia.**
