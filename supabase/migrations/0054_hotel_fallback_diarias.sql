-- ---------------------------------------------------------------------------
-- 0054 — fix: fallback hotel via produto quando contrato_info NULL
--
-- 53 operações não têm linha contrato=true em raw.vendas_excel (dados
-- históricos sem re-seed). Nesses casos, infere o hotel a partir do
-- produto vendido: 'Diárias de Hospedagem' (prioridade 1) ou
-- 'Pacote de Casamento' (prioridade 2).
-- Aprox. 41 operações ganham hotel via Diárias, +3 via Pacote; ~4 ficam NULL.
-- ---------------------------------------------------------------------------
...
-- Wrapper público inalterado (SECURITY DEFINER necessário para service_role)
