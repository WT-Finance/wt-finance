-- ---------------------------------------------------------------------------
-- 0099 — get_sumario_subsetor: adicionar n_contratos por subsetor
--
-- A pedido da diretoria, o card Comercial passa a exibir o número de
-- "Contrato de Casamento" vendidos (não o faturamento). O subsetor COMERCIAL
-- tem 3 produtos (Atualização de Contrato, Contrato de Casamento, Taxa de
-- Serviço), então n_vendas do subsetor não serve — precisa de contagem
-- específica do produto.
--
-- n_contratos = COUNT(DISTINCT fato_venda) FILTER produto_normalizado =
-- 'CONTRATO DE CASAMENTO'. Para os demais subsetores resulta 0 (o produto só
-- existe em COMERCIAL). A RPC já é chamada para período atual e YoY, então o
-- YoY da contagem vem de graça no frontend.
--
-- Mantém todo o resto idêntico ao 0077. SECURITY DEFINER, search_path = ''.
-- ---------------------------------------------------------------------------
