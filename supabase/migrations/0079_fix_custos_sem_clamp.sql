-- ---------------------------------------------------------------------------
-- 0079 — fix: get_operacoes_weddings — remove GREATEST no cálculo de custos
--
-- A migration 0078 substituiu dim.receita_bruta por v_receita (fresco), mas
-- manteve o GREATEST que clipa Custos em 0 quando resultado_caixa > v_receita.
-- Isso ainda produzia: Custos = 0 com Rec. Líq. > Rec. Bruta.
--
-- Fix: remover o GREATEST para garantir consistência aritmética:
--   Custos = v_receita − resultado_caixa (pode ser negativo)
--   Rec. Bruta − Custos = Rec. Líq. (sempre)
--
-- Custos negativo sinaliza anomalia de dados (mais caixa recebido do que
-- receita registrada em vendas), tornando o problema visível em vez de ocultá-lo.
-- ---------------------------------------------------------------------------
