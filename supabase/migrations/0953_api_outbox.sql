-- ---------------------------------------------------------------------------
-- 0953 — feat(v5.4.0/M4): outbox de callbacks AT-LEAST-ONCE (ADR-0953) — a
...
-- humano).
--
-- NUMERAÇÃO: faixa PROVISÓRIA 0950+ (o orquestrador RENUMERA este arquivo para
-- a numeração real de `supabase/migrations/` no momento da integração — não
-- decidir isso aqui). Não aplicado por este subagente (editor puro).
--
...
--     0952) passam a enfileirar 'solicitacao.criada'/'solicitacao.cancelada' NA
...
--     solic_emails_envolvidos (0952), SEM exigir_acesso/pode_ver_solic; (10)
...
-- Corpo IDÊNTICO à 0952, exceto o PERFORM logo após o INSERT bem-sucedido —
...
BEGIN
  ...
  BEGIN
    ...
    -- v5.4.0/M4 (ADR-0953): enfileira o callback DE CRIAÇÃO na MESMA transação
...
BEGIN
  ...
  -- v5.4.0/M4 (ADR-0953): enfileira o callback DE CANCELAMENTO na MESMA
...
-- padrão de admin_solic_salvar_tipo na 0950). O classificador do db-gate marca
...
BEGIN
  ...
  -- v5.4.0/M4 (ADR-0953): tipo com exige_referencia_conclusao exige uma
...
-- Idêntica à solic_emails_envolvidos (0952: autor só entra se ATIVO), MENOS
...
--    da 0952 (sem enfileiramento) — ver corpo na própria 0952.
--    (reexecutar o CREATE OR REPLACE da 0952 tal como está naquele arquivo)
