-- ---------------------------------------------------------------------------
-- 0036 — Fix: ILIKE '%' dentro de format() precisa de '%%'
--
-- format() interpreta % como especificador de formato.
-- A migração 0035 introduziu format($q$...ILIKE '%' || $5 || '%'...$q$, ...)
-- que causa "unrecognized format() type specifier '%'" em toda chamada.
-- Fix: escapar os literais % → %% dentro do template format().
-- ---------------------------------------------------------------------------
