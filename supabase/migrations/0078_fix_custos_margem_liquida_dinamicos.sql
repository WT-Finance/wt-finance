-- ---------------------------------------------------------------------------
-- 0078 — fix: get_operacoes_weddings — custos e margem_liquida computados
--         dinamicamente a partir de v_receita (fresco) em vez do valor pré-
--         computado em dim_operacao_weddings (que usa receita_bruta possivelmente
--         desatualizada e aplica GREATEST com dados antigos).
--
-- Problema: quando a dim era regenerada antes de novas vendas serem carregadas,
-- d.custos_internos = GREATEST(stale_receita - resultado_caixa, 0) = 0,
-- mas o display usa v_receita (fresco) → Rec. Líq. > Rec. Bruta com Custos = 0.
--
-- Fix: custos = GREATEST(v_receita − resultado_caixa, 0)
--      margem_liquida = resultado_caixa / v_faturamento × 100
-- ---------------------------------------------------------------------------
  ...
  v_sql := $q$
    ...
    base AS (
      SELECT
        ...
        -- custos computado dinamicamente para consistência com v_receita (fresco)
        GREATEST(COALESCE(v.receita, 0) - COALESCE(d.resultado_caixa, 0), 0)
                                                        AS d_custos_internos,
        -- margem_liquida computada dinamicamente para consistência com v_faturamento (fresco)
        CASE WHEN COALESCE(v.faturamento, 0) > 0
