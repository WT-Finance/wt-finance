...
export default async function MetasPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  const sessao = await requireArea(['metas/acompanhamento', 'metas'])
  ...
  // 'metas' = área de Cadastro/gestão (rótulo "Metas/Cadastro", v5.1.9) → libera o botão
  // "Modo de Comparação" no Acompanhamento. Quem só tem 'metas/acompanhamento' (leitura)
  // NÃO vê o botão. (Substituiu o link discreto "Comparação (Monde)" da v5.1.2.)
  const podeComparar = sessao.permissoes.includes('metas')

  return <AcompanhamentoContent data={data} podeComparar={podeComparar} />
}
