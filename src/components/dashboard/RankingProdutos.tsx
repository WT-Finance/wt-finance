...
export default function RankingProdutos({
  data, loading,
}: { data: RankingProdutoItem[]; loading: boolean }) {
  return (
    <div className="bg-white rounded-xl border border-zinc-200 p-4">
      <h2 className="text-sm font-semibold text-zinc-700 mb-3">Ranking Produtos</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-zinc-50">
            {loading
              ...
              : data.map((p, i) => (
                  <tr key={p.produto_id} className="hover:bg-zinc-50">
                    <td className="py-2 px-3 text-xs text-zinc-400">{i + 1}</td>
                    <td className="py-2 px-3 text-zinc-800 font-medium truncate max-w-[160px]">{p.nome}</td>
                    <td className="py-2 px-3 text-right text-zinc-700 tabular-nums">{fmtBRL(p.valor_total)}</td>
                    <td className="py-2 px-3 text-right text-zinc-500 tabular-nums">{p.vendas_count}</td>
                  </tr>
                ))
            }
          </tbody>
