...
### Fuso: `CURRENT_DATE`/`now()` no banco é UTC — "hoje" de São Paulo precisa de conversão
A sessão do Postgres (Supabase) roda em **UTC**. `CURRENT_DATE`/`now()::date` numa função/RPC devolvem o dia **UTC** — que a partir das ~21h de São Paulo (UTC−3) **já é o dia seguinte**. Toda lógica de "hoje" no banco (início de projeção, classificação `a_vencer`, cortes `date_trunc('month', CURRENT_DATE)`) fica **um dia adiantada** no fim da tarde/noite. **Regra:** para o "hoje" de SP dentro do banco, usar `(now() AT TIME ZONE 'America/Sao_Paulo')::date`, nunca `CURRENT_DATE` cru. (Custou caro: a projeção do Fluxo de Caixa Gerencial começava em "amanhã" — fix migration 0151, v4.22.1. É a contraparte server-side da convenção de exibir timestamptz no fuso de SP. Restam outras RPCs/views com `CURRENT_DATE` cru — follow-up sistêmico registrado no out-briefing da v4.22.1.)
