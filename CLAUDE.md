...
- **Cabeçalho de tabela = padrão único, sem caixa alta e sem negrito.** Todo `<thead>` da plataforma usa caixa **normal** (NUNCA `uppercase`/`tracking-wide`), peso `font-medium` (nunca `font-semibold`/`font-bold`), `text-xs`/`text-2xs` e cor terciária (`text-zinc-400`/`--text-muted`) — como a **Lista de Operações**; em cards-tabela, a forma canônica é `CARD_TABELA_TH`. Tabela longa que precisa de **scroll interno com cabeçalho fixo**: container `overflow-auto max-h-[...]` + `thead` `sticky top-0 z-20 bg-white [&_th]:bg-white` — o fundo opaco vai nas **células (`th`), não no `<tr>`**: em `border-collapse` (default) a faixa da borda entre as linhas do `<tr>` fica **transparente** e as linhas de dados **VAZAM** pelo cabeçalho ao rolar (a célula pinta edge-to-edge, o `<tr>` não). Divisória que gruda no cabeçalho: `[&_tr:last-child_th]:border-b [&_tr:last-child_th]:border-zinc-200` (na célula, não no `<tr>` — a borda colapsada do `<tr>` escapa na rolagem). Não há lint pegando isso — é convenção; ver DS §7. (v4.33.2: 4 telas tinham `uppercase tracking-wide` divergente — Faturamento/Emissão, Rateio, import-drawer, Composição — convergidas. v4.34.1: fundo `[&_tr]`→`[&_th]` corrige o vazamento de pixels no cabeçalho sticky.)
