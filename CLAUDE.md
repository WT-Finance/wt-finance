...
- **Ingestão de Vendas tem UM parser só** (`@/lib/carga/vendas-parser.ts`, isomórfico, sem `'use client'`). Os dois caminhos o consomem: a UI (`/admin/uploads`, via Server Actions — caminho **real**) e a API Route `upload-vendas` (hoje **vestigial/não-chamada** pela app). Paridade de colunas (incl. `operacao_propria`) é garantida pelo parser único **e** pelo SQL (staging 0118). Não recriar parser por caminho: dois parsers divergentes regrediram silenciosamente a v4.9.x (a via servidor não populava `operacao_propria`). **F2 FECHADO para a UI (v4.15.0, ADR-0111):** o caminho real agora usa o **pipeline atômico** (`limpar_staging_vendas` → `inserir_lote_staging` → `validar_carga_staging` → `promover_carga_vendas`, 0116/0118) — `getAdminClient` (service role), sem migration nova, sem timeout de 3s. Uma carga com erro não esvazia mais a base (swap numa transação; ROLLBACK preserva). O caminho antigo (`truncate_dynamic_tables`/`inserir_lote_raw`/`transform_…` soltos) e a rota vestigial **coexistem** (rollback de aplicação = reverter deployment); **remoção = fase 2** (só após ≥2 cargas reais sem incidente).
