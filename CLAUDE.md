...
- **Respiro vertical de página vem do `<main>` do AppShell (`py-8`) — fonte única.** Páginas NÃO definem `py`/`pt`/`pb` no container raiz; o container é só `px-*` em **largura TOTAL** (v5.1.1: **sem `max-w`/`mx-auto`** — a página usa toda a largura disponível do `<main>`; só o respiro horizontal varia por tela: `px-6` dashboards, `px-4` plataforma). Antes da v5.1.1 o container capava em `max-w-7xl`/`max-w-5xl mx-auto` e sobrava espaço lateral vazio (visto em Solicitações). Antes da v4.16.1 cada tela inventava o `py` (16px, 32px ou zero — conteúdo "grudado" no topo). Tela nova nasce sem `py` próprio e **sem `max-w`/`mx-auto`** no container raiz. (DS §12, v4.16.1; largura total v5.1.1.)
