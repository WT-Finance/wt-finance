...
- **O que uma chamada de RPC do Supabase devolve é um *thenable*, NÃO uma Promise — `.catch()` nele estoura em RUNTIME e o `tsc` não vê.** O builder do `supabase-js` implementa `.then` (por isso `await` e `Promise.all`/`allSettled` funcionam), mas **não** `.catch`/`.finally`: encadear `rpc(...).catch(() => fallback)` dá `TypeError: ... .catch is not a function` só quando a página roda. Trate falha pelo `status` de cada item de um `Promise.allSettled` (o padrão já usado nas páginas com fetch paralelo), ou embrulhe em `Promise.resolve(...)` antes de encadear. (Custou caro: a página da DRE quebrou inteira na v5.3.0 com gates verdes — pego só na conferência visual.)
