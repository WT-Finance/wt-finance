...
- **Container rolável INTERNO (lista/painel dentro da página) usa `<ScrollAutoHide>` (`@/components/shared/scroll-auto-hide`) — regra de escoteiro (v4.40.0/DS §Barras de rolagem):** a barra flutua em overlay (nativa escondida por `.scrollbar-none`, thumb que aparece ao rolar/hover e some sozinho ~1,2s) e, por ser overlay, NÃO desloca o conteúdo (dispensa `scrollbar-gutter`). Ao **encostar** num container rolável interno que ainda usa `overflow-y-auto` cru, migre-o para `<ScrollAutoHide>` (migração incremental, byte-equivalente). NÃO vale para o `<main>` do AppShell (esse é o scroll do documento, mantém nativa + `scrollbar-gutter: stable`, ver bullet acima). (Origem: sidebar; aplicado no Acervo.)
