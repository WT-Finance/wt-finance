...
- **Primitivos (M2):** `<Gauge>` (medidor semicírculo com tick-de-pace); `<NavGroup>` genérico na sidebar (Performance/Financeiro/Metas); `kpi-principal-drawer` realocado de `weddings/`→`performance/`. *(Consolidação das 3 pills de período deferida — a variante de Weddings é Context, migrar arriscaria regressão; Metas reusa a base.)*
- **Acompanhamento (M3):** `/metas` — card Group (gauge grande neutro + 3 KPIs) + 3 cards setoriais (gauge na cor do setor) + gráfico "Ritmo do período" (realizado acumulado × meta acumulada tracejada, "hoje" e esperado). Group computado. Tema group.
- **Cadastro (M4):** `/metas/cadastro` — grade anual 12×3 (Meta VT + % Rec) com Group computado ao vivo, autosave por célula (reverte em erro) e auditoria da última alteração.
