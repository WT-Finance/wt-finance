...
MINOR · **API Externa do módulo de Solicitações (núcleo genérico + admin de chaves).** Plataformas internas passam a criar e cancelar solicitações no Janus via **chave de API**, e a receber **callbacks de status** — categoria NOVA de confiança (escrita autorizada por chave, sem requireArea humano; ADR-0950 provisório). O Janus é o dono do formato: o **cadastro de tipos gera o contrato** — slug estável no tipo, **CHAVE ESTÁVEL por campo** (sobrevive ao apaga-e-recria do editor; corrige a fragilidade dos IDs recriados — ADR-0951), flags `exposto_via_api`/`exige_referencia_conclusao` e lista de roles permitidas; retrofit dos tipos existentes via RPC service-only (migration segue aditiva). **Chaves por plataforma** (`/admin/chaves-api`): segredo só como hash sha256 (exibido uma única vez), whitelist de tipos, usuário-robô como autor (proveniência), revogação irreversível, log de chamadas. **Validação COMPARTILHADA** (`app.solic_validar_e_snapshotar`, extraída verbatim — incl. regra de data v4.19): a UI e a API recusam exatamente o mesmo; RPC-irmã `criar_solicitacao_externa` (idempotência obrigatória com índice único e proteção de corrida; destinatário do disparo validado SEM fallback e ecoado — ADR-0952) + `cancelar_solicitacao_externa` (só da própria chave, só aberta; conflito reportado). Rotas `/api/externo/{tipos, solicitacoes, solicitacoes/{id}/cancelar, outbox/processar}` com auth própria (API_AUTH_PROPRIA por prefixo), erros estruturados e limite de payload. **Outbox de callbacks at-least-once** (ADR-0953): 4 eventos enfileirados NA transação da movimentação (só origem externa), entrega com retry/backoff (pg_cron 5min + inline nas rotas), movimentação JAMAIS quebra por entrega; `solic_concluir` ganha referência externa obrigatória quando o tipo exige (viaja no callback; UI do drawer/board pede o campo). Seed do primeiro tipo exposto ("Abatimento de créditos", TARS/CRM) com roles vazias (inerte até configuração) + **documento de contrato do integrador** (`docs/api-externa-solicitacoes.md`). Migrations **0950–0954 PROVISÓRIAS** (faixa reservada; renumeração no merge, após a v5.3.0) — todas aditivas, aplicadas com backup-gate; suíte de contrato 24/24 ao vivo. **Merge SOMENTE após a v5.3.0.**
