...
- **Coluna Valor da revisão** em formato **contábil** (`ValorContabil`); **coluna Boleto alargada** (o "ver boleto" não quebra linha); nos modais de resultado, a **coluna Pessoa trunca** com reticências + `title`. A **tabela de revisão** perde o `min-width` (cabe sempre no container — sem barra de rolagem horizontal; a coluna Pessoa encolhe e quebra com `break-words`) e as colunas Vencimento/Fatura/Nota foram enxutas para dar respiro à Pessoa.
