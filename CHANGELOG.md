...
- **Bug corrigido, reportado com a tela na mão:** marcar uma operação no filtro fazia a página **saltar para o topo**. O App Router rola para o topo em toda navegação por default e o `dropdown-operacao` fazia `router.push(url)` sem `{ scroll: false }` — numa interação de múltipla escolha, isso arranca o usuário do controle a cada clique. **O defeito é do padrão:** 7 outros filtros que navegam no lugar têm o mesmo problema (lista no out-briefing), fora do escopo desta versão por disciplina.
